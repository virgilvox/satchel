...
use satchel_rag::ingest::{self, IngestConfig};
use std::io::Write;
...
async fn test_ingest_single_markdown_file() {
    ...
    ingest::ingest_path(&file_path, &db, &embedder, &config())
        .await
        .unwrap();
...
async fn test_ingest_directory_multiple_files() {
    ...
    ingest::ingest_path(dir.path(), &db, &embedder, &config())
        .await
        .unwrap();
...
async fn test_ingest_dedup_by_hash() {
    ...
    ingest::ingest_path(&dir.path().join("first.md"), &db, &embedder, &config())
        .await
        .unwrap();
    ...
    ingest::ingest_path(&dir.path().join("second.md"), &db, &embedder, &config())
        .await
        .unwrap();
